...
}, [autoRefreshEnabled, activeTab, selectedReportPeriod, comprehensiveDashboard, realtimeMetrics]);
...
// REPLACE YOUR EXISTING useEffect AROUND LINE 800 WITH THIS ENHANCED VERSION
  ...
  loadAllData();
}, []); // Empty dependency array is fine for initial load // Empty dependency array is fine for initial load
  ...
  }, [calendarView, showCalendarModal, activeTab, loadCalendarData]);
        ...
        <div className="nav-content">
          <div className="nav-tabs nav-tabs-professional">
            {[
              ...
              { id: 'financial', label: 'Financial', icon: '💰' }, // NEW
              ...
              <button
                ...
                className={`nav-tab nav-tab-professional ${activeTab === id ? 'active' : ''}`}
              >
      ...
      <main className="main-content main-content-professional">
              ...
              <div className="messages-panel">
                {selectedConversation ? (
                  <div className="conversation-view">
                    <div className="conversation-header">
                      <h3>Conversation with {selectedConversation.participants.map(p => p.name).join(', ')}</h3>
        ...
        {activeTab === 'pricing' && (
          <div className="pricing-section">
            ...
            {pricingData && pricingData.units ? (
              <div className="pricing-grid">
                {pricingData.units.map(unit => (
                  <div key={unit.id} className="pricing-card">
                    ...
                    <div className="pricing-details">
                      <div className="price-range">
                        ...
                        <span className="range-value">
                          US${unit.priceRangeUSD.min} - US${unit.priceRangeUSD.max}
                        </span>
                      </div>
                      <div className="pricing-breakdown">
                        {Object.entries(unit.basePricing).map(([guestType, pricing]) => (
                          <div key={guestType} className="price-item">
                            <span className="guest-type">{guestType.replace('guest', '')} Guest{guestType !== 'guest1' ? 's' : ''}:</span>
                            <span className="price-values">
                              <span className="usd-price">US${pricing.USD}</span>
              ...
              <div className="fallback-pricing">
                <div className="pricing-grid">
                  {properties.map(property => (
                    <div key={property.id} className="pricing-card">
                      <div className="pricing-header">
                        <h3>{property.name}</h3>
                        <span className="last-updated">Current Pricing</span>
                      </div>
                      <div className="pricing-details">
                        <div className="price-range">
                          <span className="range-label">Base Price:</span>
                          <span className="range-value">
                            {formatCurrency(property.basePriceLKR || property.pricing?.basePriceLKR, 'LKR')}
                          </span>
                        </div>
                        <div className="pricing-breakdown">
                          <div className="price-item">
                            <span className="guest-type">Per Night:</span>
                            <span className="price-values">
                              <span className="usd-price">US${Math.round((property.basePriceLKR || 30000) / currencyRate)}</span>
                              <span className="lkr-price">LKR {(property.basePriceLKR || 30000).toLocaleString()}</span>
                            </span>
                          </div>
                        </div>
                      </div>
            ...
            <div className="exchange-rate-card">
              ...
              <p>1 USD = {currencyRate.toFixed(2)} LKR</p>
              <small>LKR prices are automatically calculated based on USD rates</small>
            </div>
          </div>
        )}
            ...
            {calendarData && calendarData.calendar ? (
              <div className="calendar-display">
                {calendarData.calendar.map(unitCalendar => (
                  <div key={unitCalendar.unitId} className="unit-calendar">
                    <h3>{unitCalendar.unitName}</h3>
                    <div className="calendar-grid">
                      {unitCalendar.dates.map(day => (
                          <div
                            key={day.date}
                            className={getAvailabilityStyle(day.available, day.source)}
                            title={`${day.date} - ${day.available ? 'Available' : 'Unavailable'} - ${day.source || 'Normal'}`}
                            onClick={() => {
                              // Open calendar modal with this date selected
                          ...
                          {day.pricing && (
                            <div className="calendar-day-price">
                              ${day.pricing.USD}
                            </div>
                          )}
                          {day.hasOverride && (
                            <div className="calendar-day-override">
                              🏷️
                            </div>
                          )}
                          {day.guestName && (
                            <div className="calendar-day-guest">
                              {day.guestName.substring(0, 10)}...
                            </div>
                          )}
                        </div>
                      ))}
              ...
              <div className="calendar-placeholder">
                <p>Click "Load Calendar" to view availability and pricing for selected date range.</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="header-actions" style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{
              background: autoRefreshEnabled ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 'white',
...
>
  {isUpdating ? '🔄 Syncing...' : '🎯 Force Sync Charts'}
          ...
          <button
            ...
            style={{
              marginRight: '1rem',
              background: isUpdating ? '#9ca3af' : 'white',
          ...
          {Object.entries(dashboardCharts).map(([chartName, isVisible]) => (
            <button
              ...
              style={{
                ...
                borderRadius: '6px',
                background: isVisible ? '#3b82f6' : 'white',
            ...
            {advancedAnalytics && (
              <div className="advanced-analytics-section">
                ...
                <div className="analytics-grid" style={{
                  ...
                  // ADD THIS AFTER THE FORECASTING CARD (around line 7400)
// Look for: {/* Advanced Analytics with Forecasting */}
// Add this new chart section:
...
// ADD THIS WHERE YOU DISPLAY PAYMENT METHODS (in the dashboard charts section)
            ...
            <div className="modal-content">
              <div className="revenue-form">
                ...
                {revenueForm.type === 'services' && (
                  <div className="service-examples" style={{
                    padding: '1rem',
                    background: 'linear-gradient(135deg, #ecfdf5 0%, #d1fae5 100%)',
                    borderRadius: '8px',
        ...
        <div className="revenue-filters" style={{
          ...
          <div className="filter-tabs">
            {[
              ...
              <button
                key={filter.id}
                onClick={() => setRevenueFilters(prev => ({ ...prev, type: filter.id }))}
                className={`filter-tab ${revenueFilters.type === filter.id ? 'active' : ''}`}
              >
                {filter.label}
        ...
        <div className="revenue-list">
          {getFilteredAndSortedRevenue().length === 0 ? (
            ...
            <div className="revenue-table-container">
              <table className="revenue-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                <thead>
                  <tr>
                    <th style={{ background: '#f8fafc', color: '#374151', fontWeight: '600', padding: '1rem 0.75rem', textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Date</th>
                    <th style={{ background: '#f8fafc', color: '#374151', fontWeight: '600', padding: '1rem 0.75rem', textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Description</th>
                    <th style={{ background: '#f8fafc', color: '#374151', fontWeight: '600', padding: '1rem 0.75rem', textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Type</th>
                    <th style={{ background: '#f8fafc', color: '#374151', fontWeight: '600', padding: '1rem 0.75rem', textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Guest</th>
                    <th style={{ background: '#f8fafc', color: '#374151', fontWeight: '600', padding: '1rem 0.75rem', textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Amount</th>
                    <th style={{ background: '#f8fafc', color: '#374151', fontWeight: '600', padding: '1rem 0.75rem', textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Status</th>
                    <th style={{ background: '#f8fafc', color: '#374151', fontWeight: '600', padding: '1rem 0.75rem', textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div className="expense-filters">
              ...
              <div className="filter-tabs">
                {[
                  ...
                  <button
                    key={filter.id}
                    onClick={() => setExpenseFilters(prev => ({ ...prev, status: filter.id }))}
                    className={`filter-tab ${expenseFilters.status === filter.id ? 'active' : ''}`}
                  >
                    {filter.label}
                    <span className="count-badge">{filter.count}</span>
                  </button>
